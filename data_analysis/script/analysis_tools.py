...
def plot_2D_bins(total_complete, bb1_pbind, bb2_pbind, bb3_pbind, all_bins):
    ...
    val_1_2, val_1_3, val_2_3 : array
        array of the probability of forming binders for combinations of P(bind) bins
    '''
    ...
    for ind_1, ind_2, binder_rate in zip(ab['bb1_bins'], ab['bb2_bins'], ab['bind']):
        val_1_2[n_bins-1-ind_2, ind_1] = binder_rate

    # Calculate probability of forming binders compound for combinations of P(bind) bins in positions 2 and 3
    bc = total_complete.groupby(['bb2_bins', 'bb3_bins'], as_index=False)['bind'].mean()
    val_2_3 = np.zeros((n_bins,n_bins))
    for ind_2, ind_3, binder_rate in zip(bc['bb2_bins'], bc['bb3_bins'], bc['bind']):
        val_2_3[n_bins-1-ind_3, ind_2] = binder_rate

    # Calculate probability of forming binders compound for combinations of P(bind) bins in positions 1 and 3
    ac = total_complete.groupby(['bb1_bins', 'bb3_bins'], as_index=False)['bind'].mean()
    val_1_3 = np.zeros((n_bins,n_bins))
    for ind_1, ind_3, binder_rate in zip(ac['bb1_bins'], ac['bb3_bins'], ac['bind']):
        val_1_3[n_bins-1-ind_3, ind_1] = binder_rate
...
def plot_hdbscan_interactive(bb_pbind, params, transform, bb_pos):
    '''
...
def plot_cluster_combos(total_compounds, bb1_pbind, bb2_pbind, bb3_pbind, trans_bb1, trans_bb2, trans_bb3):
    ...
    ab_mat, ac_mat, bc_mat : array
        array containing the pairwise probability of forming binders for combinations of building block clusters in each position
    ...
    # Calculate pairwise probability of forming binders for building block clusters
...
def gen_random_cluster(bb_pbind, bb_pos):
    '''
